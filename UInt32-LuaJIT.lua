...
local function check_type(integer)
    ...
    elseif type(integer) == "number" then
        if number_type(integer) == "integer" then
            i = integer
        elseif number_type(integer) == "float" then
...
function UInt32.binary_and(u1, u2)
    ...
    return UInt32:new(bit.band(u1.value, u2.value))
end
...
function UInt32.binary_or(u1, u2)
    ...
    return UInt32:new(bit.bor(u1.value, u2.value))
end
...
function UInt32.binary_xor(u1, u2)
    ...
    return UInt32:new(bit.bxor(u1.value, u2.value))
end
...
function UInt32.binary_not(u1)
    ...
    return UInt32:new(bit.bnot(u1.value))
end

function UInt32.shift_left(u1, i1)
    assert(type(u1) == "table", "Error: argument 1 must be UInt32.")
    assert(u1.__name == UInt32.__name, "Error: argument 1 must be UInt32.")
    assert(number_type(i1) == "integer", "Error: argument 2 must be integer.")
    assert(i1 > 0 and i1 <= 32, "Error: argument 2 must be in the range 1 - 32.")

    return UInt32:new(bit.lshift(u1.value, i1))
end
...
function UInt32.shift_right(u1, i1)
    ...
    return UInt32:new(bit.rshift(u1.value, i1))
end
...
function UInt32.divide(u1, u2)
    ...
    return UInt32:new(math.floor(u1.value / u2.value))
end
UInt32.__div = UInt32.divide
